    ...
    <Link href={`/product/${product.id}`} aria-label={`${product.name} - ${formatPrice(product.price)}`}>
      ...
      >
        <div className="aspect-square relative bg-[#1A1A22] overflow-hidden">
          {/* Subtle gradient glow behind the shoe */}
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(255,46,136,0.04)_0%,transparent_70%)] group-hover:bg-[radial-gradient(ellipse_at_center,rgba(255,46,136,0.08)_0%,transparent_70%)] transition-all duration-700" />
          {product.image_url ? (
            <Image
              src={product.image_url}
              alt={product.name}
              fill
              className="object-contain p-6 relative z-[1] transition-all duration-700 ease-out group-hover:scale-110 group-hover:rotate-[-2deg]"
          ...
          <div className="absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-[#141418] to-transparent z-[2] pointer-events-none" />
          {/* Condition badge */}
